...
class Problem:
    ...
    def create(self, arr=None):
        ...
        if arr is None:
            arr = np.random.uniform(
                self.domain.MIN_VAL, self.domain.MAX_VAL, self.size
            )
        else:
            arr = np.asarray(arr)
        o.setArray(arr)
    ...
    def randomVal(self):
        return self.domain.MIN_VAL + random.random() * (
            self.domain.MAX_VAL - self.domain.MIN_VAL
        )
...
class ProblemData:
    ...
    def setArray(self, np_arr):
        # 下限と上限でまるめる
        np_arr = np.where(
            np_arr < self.domain.MIN_VAL, self.domain.MIN_VAL, np_arr
        )
        np_arr = np.where(
            np_arr > self.domain.MAX_VAL, self.domain.MAX_VAL, np_arr
        )
        self.np_arr = np_arr
        self.score = None
