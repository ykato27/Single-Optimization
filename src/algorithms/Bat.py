...
class Bat(IAlgorithm):
    ...
    def init(self, problem):
        ...
        for _ in range(self.bat_max):
            ...
            if (
                self.best_bat is None
                or self.best_bat.getScore() < o.getScore()
            ):
                self.best_bat = o
    ...
    def _calcPulse(self, count):
        return self.pulse_convergence_value * (
            1 - math.exp(-self.pulse_convergence_speed * count)
        )
    ...
    def step(self):
        ...
        for bat in self.bats:
            ...
            frequency = (
                self.frequency_min
                + (self.frequency_max - self.frequency_min) * random.random()
            )
