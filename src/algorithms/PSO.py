...
class PSO(IAlgorithm):
    ...
    def init(self, problem):
        ...
        for _ in range(self.particle_max):
            ...
            # 初期加速度
            v = [
                (problem.MAX_VAL - problem.MIN_VAL) * random.uniform(-1, 1)
    ...
    def _updateBest(self, particle):
        ...
        # パーソナルベストの更新
        if (
            particle["personal"] is None
            or particle["personal"].getScore()
            < particle["particle"].getScore()
        ):
        ...
        if (
            ...
            or self.global_best.getScore() < particle["particle"].getScore()
        ):
