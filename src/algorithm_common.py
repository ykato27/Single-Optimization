...
class AlgorithmCommon:
    ...
    def arithmetic_sequence_sum_inverse(val, start=1, diff=1):
        ...
        t = (
            diff
            - 2 * start
            + math.sqrt((2 * start - diff) ** 2 + 8 * diff * val)
        )
        return t / (2 * diff)
    ...
    def mantegna(beta):
        ...
        t = t / (
            AlgorithmCommon.gamma((1 + beta) / 2)
            * beta
            * 2 ** ((beta - 1) / 2)
        )
        siguma = t ** (1 / beta)
        ...
        u = (
            AlgorithmCommon.random_normal() * siguma
        )  # 平均0 分散siguma^2 の正規分布に従う乱数
            ...
            s = (
                (
                    (
                        (
                            (
                                (-0.000592166437354 * v + 0.0000697281375837)
                                * v
                                + 0.00078403922172
                            )
                            * v
