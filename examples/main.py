        ...
        if alg_cls.__name__ == GA.__name__:
            alg = GA(
                ...
                trial.suggest_categorical(
                    "select_method", ["ranking", "roulette"]
                ),
            ...
            alg = Harmony(
                ...
                trial.suggest_categorical(
                    "enable_bandwidth_rate", [False, True]
                ),
...
def main():
    ...
    for prob_cls in probs:
        for alg_cls in algs:
            path = os.path.join(
                tmp_dir,
                "{}_{}.json".format(prob_cls.__name__, alg_cls.__name__),
            )
            ...
            study = optuna.create_study(
                storage="sqlite:///"
                + os.path.join(tmp_dir, "optuna_study.db"),
                direction="maximize",
            )
            study.optimize(
                objective_degree(prob_cls, alg_cls, timeout=2),
                n_trials=100,
                n_jobs=4,
            )
            ...
            data = joblib.Parallel(n_jobs=4, verbose=0)(
                [joblib.delayed(multi_run)(params) for _ in range(100)]
            )
