...
def create(D, N):
    ...
    algs = [
        ...
        Harmony(
            N,
            bandwidth=1.0,
            enable_bandwidth_rate=True,
            select_rate=0.4,
            change_rate=0.9,
        ),
        PSO(
            N, inertia=0.2, global_acceleration=0.2, personal_acceleration=0.2
        ),
...
def anime2(func, alg):
    ...
    def plot(i):
        ...
        plt.plot(
            func_x, func_y, label=func.__class__.__name__, linewidth="0.5"
        )
        plt.plot(
            x_pos[i], y_score[i], "o", color="orange", markeredgecolor="black"
        )
        plt.plot(
            max_pos[i], max_score[i], "o", color="red", markeredgecolor="black"
        )
    ...
    path = os.path.join(
        tmp_dir,
        "{}_{}_2.gif".format(func.__class__.__name__, alg.__class__.__name__),
    )
    print(path)
...
def anime3(func, alg):
    ...
    def plot(i):
        ...
        plt.imshow(func_y, interpolation="nearest", cmap="jet", extent=extent)
        plt.plot(
            pos1[i], pos2[i], "o", color="orange", markeredgecolor="black"
        )
        plt.plot(
            max_pos1[i], max_pos2[i], "o", color="red", markeredgecolor="black"
        )
        plt.title("step={}".format(i))
    ...
    path = os.path.join(
        tmp_dir,
        "{}_{}_3.gif".format(func.__class__.__name__, alg.__class__.__name__),
    )
    print(path)
    ani.save(path, writer="imagemagick")
    # plt.show()
